...
execute unless score @s blink.count matches 1000 if block ~ ~ ~ air positioned ^ ^ ^1 run function blink:blink
#execute unless score @s blink.cooldown matches ..1 unless block ~ ~ ~ air run 
scoreboard players operation @s[scores={blink.cooldown=0}] blink.cooldown = #cooldown blink.config
